...
The first hosted rerun completed in [run 34796201326](https://github.com/mbodge/theartist/actions/runs/34796201326) and saved its checkpoint. Its diagnostics passed all 20 scenarios and 68 groups, but the experiment required the original laptop architecture and a supplemental console check failed on the favicon. Publication was correctly blocked. The founder's context now describes the actual Docker resource limits, and launcher guidance requests a new prospective hosted-environment contract, preserving both prior non-passes. For this launch repair, the configured daily workshop ceiling is three jobs and the publication ceiling is eight per kind; ordinary stage-call and time limits remain in force.
